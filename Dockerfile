                                     ...
                                     VmbpWalk *walk, unsigned pending_index)
{
    ...
    if (vmbp_read_u64(walk, pending->dll_handle_ptr, &base) && base &&
        base < VMBP_USER_END && !(base & 0xffff)) {
        bool image_ok = vmbp_match_image(walk, pending->profile, base);
        bool site_ok = image_ok &&
            vmbp_match_site(walk, pending->profile,
                            base + vmbp_profiles[pending->profile].in_rva);

        /* LdrLoadDll's fourth argument is the caller's final module-handle
         * output.  Once it receives a sane 64-KiB-aligned user image base,
         * stop watching the caller's stack slot immediately.  The execution
         * breakpoint performs the full PE/thunk/port/magic validation again
         * before emulating anything, so setup-time validation is diagnostic
         * only and must not create a race with the first UMD call.
         */
        ...
        if (vmbp_handle_miss_logs++ < 12) {
            fprintf(stderr, "vmport-bp: handle-write target=%s cr3=0x%" PRIx64
                            ...
                            " (waiting for aligned module base)\n",
